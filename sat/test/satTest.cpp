...
initCircuit()
{
   ...
   gates.push_back(new Gate(1));  // gates[0]
   gates.push_back(new Gate(2));  // gates[1]
   gates.push_back(new Gate(3));  // gates[2]
   gates.push_back(new Gate(4));  // gates[3]
   gates.push_back(new Gate(5));  // gates[4]
   cout << "gate init....." << endl;
...
genProofModel(SatSolver& s)
{
   ...
   cout << "enter proof model..." << endl;
   for (size_t i = 0, n = gates.size(); i < n; ++i) {
      Var v = s.newVar();
      gates[i]->setVar(v);
   }
   cout << "finish set var......" << endl;
   ...
   // s.addAigCNF(gates[2]->getVar(), gates[1]->getVar(), false, gates[1]->getVar(), true);
   // s.addAigCNF(gates[4]->getVar(), gates[3]->getVar(), false, gates[3]->getVar(), true);
   s.addAigCNF(gates[5]->getVar(), gates[2]->getVar(), false, gates[4]->getVar(), true);
}
...
int main()
{
   ...
   // newV = solver.newVar();
   // solver.addAigCNF(newV, gates[1]->getVar(), false, gates[2]->getVar(), true);
   // solver.assumeRelease();  // Clear assumptions
   solver.assumeProperty(gates[gates.size()-1]->getVar(), true);  // k = 1
   result = solver.assumpSolve();
   reportResult(solver, result);
}
